...
from experiment2 import experiment_svm_linear
...
EXPERIMENT_NAME = "crossvalidation2"

COMBINATIONS = set([("welch_32",1),("dwt",0.01)])
...
def main():
	for feature, c in COMBINATIONS:
		try:
			print("Computing " + feature  + " SVM-linear C = " + str(c) + " cross validation")
			...
			for i in range(len(splits)):
				...
				if res != None:
					if SAVE_RESULTS:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_results_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
						joblib.dump(res,path)

					if SAVE_RESULTS_TXT:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_results_" + str(i+1) + ".txt"
						path = os.path.join(EXPERIMENT_NAME,filename)
						save_txt(res,path)

					if SAVE_MODEL:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_model_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
						joblib.dump(model,path)

					if SAVE_PCA:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_pca_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
			...
			if SAVE_RESULTS:
				filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_results_averaged"
				path = os.path.join(EXPERIMENT_NAME,filename)
				joblib.dump(average_report,path)

			if SAVE_RESULTS_TXT:
				filename = EXPERIMENT_NAME + "_" + feature + " svm_lin_c_" + dots_to_underscores(str(c)) + "_results_averaged.txt"
				path = os.path.join(EXPERIMENT_NAME,filename)
				save_txt(average_report,path)

		except Exception as e:
			print("Error during " + EXPERIMENT_NAME + " " + feature  + " SVM-linear C = " + dots_to_underscores(str(c)))
