...
from experiment3 import experiment_svm_rbf
...
EXPERIMENT_NAME = "crossvalidation3"

COMBINATIONS = set([("welch_32",10,10),("dwt_stat",0.01,0.1)])
...
def main():
	for feature, c, gamma in COMBINATIONS:
		try:
			print("Computing " + feature  + " SVM-rbf C = " + str(c) + " gamma = " + str(gamma) + " cross validation")
			...
			for i in range(len(splits)):
				...
				if res != None:
					if SAVE_RESULTS:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) + "_gamma_" + dots_to_underscores(str(gamma)) + "_results_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
						joblib.dump(res,path)

					if SAVE_RESULTS_TXT:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) +  "_gamma_" + dots_to_underscores(str(gamma)) + "_results_" + str(i+1) + ".txt"
						path = os.path.join(EXPERIMENT_NAME,filename)
						save_txt(res,path)

					if SAVE_MODEL:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) +  "_gamma_" + dots_to_underscores(str(gamma)) + "_model_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
						joblib.dump(model,path)

					if SAVE_PCA:
						filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) +  "_gamma_" + dots_to_underscores(str(gamma)) + "_pca_" + str(i+1)
						path = os.path.join(EXPERIMENT_NAME,filename)
			...
			if SAVE_RESULTS:
				filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) + "_gamma_" + dots_to_underscores(str(gamma)) + "_results_averaged"
				path = os.path.join(EXPERIMENT_NAME,filename)
				joblib.dump(average_report,path)

			if SAVE_RESULTS_TXT:
				filename = EXPERIMENT_NAME + "_" + feature + " svm_rbf_c_" + dots_to_underscores(str(c)) +  "_gamma_" + dots_to_underscores(str(gamma)) + "_results_averaged.txt"
				path = os.path.join(EXPERIMENT_NAME,filename)
				save_txt(average_report,path)

		except Exception as e:
			print("Error during " + EXPERIMENT_NAME + " " + feature  + " SVM-rbf C = " + dots_to_underscores(str(c)) + "_gamma_" + dots_to_underscores(str(gamma)))
